...
## 🔔 Сценарий: Уведомления в веб‑интерфейсе

1. После входа приложение подписывается на счётчик непрочитанных из `users/{uid}/notifications` (read==false).
2. В шапке слева отображается колокол с бейджем количества.
3. По клику открывается список (3 последних), кнопка «Показать больше» подгружает по 10.
4. Клик по уведомлению помечает его как прочитанное и открывает событие.
5. Кнопка «Отметить все» помечает все как прочитанные и сразу обновляет UI.

Создание уведомлений:
- При создании/обновлении события (участники) уведомления формируются автоматически для реальных пользователей; «custom_*» игнорируются.