...
btn-{size} - sm, md, lg
btn-icon - для иконочных кнопок
btn-{function} - close, back, delete, add, edit, save, cancel
```

### Примеры:
- `btn btn-icon btn-close` - кнопка закрытия
- `btn btn-primary btn-lg` - большая основная кнопка
- `btn btn-danger btn-delete` - кнопка удаления
- `btn btn-icon btn-back` - кнопка назад

### Единые размеры:
- **Иконочные кнопки:** 44x44px (мобильный стандарт)
- **Маленькие кнопки:** высота 36px
